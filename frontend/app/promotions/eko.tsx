...
export default function EkoScreen() {
    ...
    const handleGenerate = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.ai.generateWithEko(prompt);
            // The backend returns { suggestions: [...] } or { content: ... }
            // Let's assume the backend will return { content: "..." } for simple prompt
            // Or we adapt the UI to handle suggestions.
            // For now, let's map the first suggestion if structure differs.
            const content = response.content || (response.suggestions && response.suggestions[0]?.description) || "No result";
            setResult(content);
        } catch (error) {
            console.error(error);
