...
import { Users, TrendingUp, Search, Filter, Bell, UserPlus } from 'lucide-react';
  ...
  return (
    <div className="h-full bg-transparent pb-20 animate-fade-in relative">
        ...
        <header className="px-6 pt-10 pb-6 flex items-center justify-between relative z-10">
            ...
            <button className="w-10 h-10 rounded-full flex items-center justify-center glass-panel shadow-sm border border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 hover:text-primary transition-colors">
                <Bell size={20} />
            </button>
        </header>
            ...
            <div className="glass-panel backdrop-blur-md border border-gray-200 dark:border-white/5 rounded-2xl p-5 relative overflow-hidden group">
                ...
                <div className="relative z-10 flex justify-between items-end">
                    <div>
                        ...
                        <h2 className="text-3xl font-bold text-gray-900 dark:text-white">12,450</h2>
        ...
        {/* Search Bar */}
        <section className="px-6 mb-4 flex gap-3 relative z-10">
                    ...
                    {[
                        ...
                        <div key={i} className="grid grid-cols-12 gap-2 px-4 py-3 items-center hover:bg-white/40 dark:hover:bg-white/5 transition-colors cursor-pointer group">
                            <div className="col-span-6 flex items-center gap-3 overflow-hidden">
                                {user.img ? (
                                    <img src={user.img} alt={user.name} className="w-8 h-8 rounded-full object-cover ring-2 ring-white dark:ring-surface-dark" />
                                ) : (
        ...
        {/* FAB */}
        <button className="fixed bottom-24 right-6 w-14 h-14 bg-gradient-to-tr from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white shadow-lg shadow-purple-500/40 hover:scale-105 active:scale-95 transition-transform z-30 border-2 border-white dark:border-surface-dark">
            <UserPlus size={24} />
        </button>
    </div>
  );
